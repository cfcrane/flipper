...
/*This program splits its input ABH genotypes file into separate subgroups (domains) on the basis of a preliminary subset map.*/
/*This program is intended for a single linkage group with more than 50000 markers.*/
/*This program is altered from mapextlgs.cpp to use domains instead of linkage groups.*/
...
int main(int argc, char *argv[]) {
  ...
  int i, j, k, lg, nlgs, N, targetgroup, rval, cllength, ierror, maxmaplinelength, maxdatlinelength, regresultkey, regresultlg, getflag, currlength;
  int refnLoci, *refdomains, ndomains, maxrefnamelength, maxoutfilenamelength, plantsTotal, mindiff, currdiff, savedgroup, maxdatnamelength;
  int currgroup, targetcount, targetinc, domain, gs, v;
  double recombfrac, minrf, nmissing;
  ...
  while (fgets(cfgline, cllength, CFG)) {
    ...
    if (strcmp(varname, "N") == 0) { /*N = number of pieces to divide the target linkage group into*/
      N = (int)strtol(value, (char **)NULL, 10);
      ndomains = N + 1;
      ierror = 0;
    }
    if (strcmp(varname, "targetgroup") == 0) {
      targetgroup = (int)strtol(value, (char **)NULL, 10);
      ierror = 0;
    }
  ...
  while (fgets(line, maxmaplinelength, INM)) {
    ...
    if (getflag == 1) {
      if (regexec(&patternlg, line, 0, NULL, 0)) {
        refnLoci++;
        if (currgroup == targetgroup) targetcount++;
        token = strtok(line, " |\t\n,");
        if (strlen(token) > maxrefnamelength) maxrefnamelength = strlen(token);
        //printf("nlgs = %d refnLoci = %d token = %s\n", nlgs, refnLoci, token);
        //Reinstate reflgs and refnames, even though refdomains gets output.
      }
      else {
        nlgs++;
        currgroup++;
  ...
  targetinc = targetcount / N; //integer division, truncated
  printf ("Ending value of getflag is %d nlgs = %d targetgroup = %d targetcount = %d targetinc = %d refnLoci = %d maxrefnamelength = %d\n", getflag, nlgs, targetgroup, targetcount, targetinc, refnLoci, maxrefnamelength);
  fflush(NULL);
  ...
  refdomains = (int *) calloc(refnLoci + 5, sizeof(int));
  for (i = 0; i < refnLoci + 5; i++) refdomains[i] = -1;
  ...
  while (fgets(line, maxmaplinelength, INM)) {
    ...
    if (getflag == 1) {
      if (regexec(&patternlg, line, 0, NULL, 0)) {
        ...
        if (lg == targetgroup) {
          v = gs / targetinc;
          //printf("Diag: j = %d lg = %d v = %d gs = %d targetinc = %d N = %d\n", j, lg, v, gs, targetinc, N);
          if (v < N) domain = v;
          else domain = N - 1;
          gs++;
        }
  ...
  fflush(NULL);
  for (i = 0; i < refnLoci; i++) printf("line 213 of program: refnames[%d] = %s refdomains[%d] = %d\n", i, refnames[i], i, refdomains[i]);
    ...
    if (!regexec(&patternast, line, 0, NULL, 0)) {
      ...
      for (k = 0; k < refnLoci; k++) {
        if (strncmp(datname, refnames[k], maxdatnamelength) == 0) { //fill in refchdata[k]
          printf("Matching name found, datname = %s refnames[%d] = %s domain = %d\n", datname, k, refnames[k], refdomains[k]);
          fflush(NULL);
          fgets(line, maxdatlinelength, DAT);
  ...
  for (i = 0; i < refnLoci; i++) printf("line 273 of program: refnames[%d] = %s refdomains[%d] = %d refchdata[%d] = %s\n", i, refnames[i], i, refdomains[i], i, refchdata[i]);
  OUD = (FILE **) malloc(ndomains * sizeof(FILE *));
  for (i = 0; i < N; i++) {
  ...
  for (i = 0; i < N; i++) fprintf(OUD[i], "%s%s%s", header1, header2, header3); //Correct nLoci later.
  fflush(NULL);
  k = 0;
  while (fgets(line, maxdatlinelength, DAT)) {
    if (!regexec(&patternast, line, 0, NULL, 0)) {
      ...
      minrf = 1; //This would happen if every character differed between datum and reference.
      savedgroup = -1;
      if ((pos = strchr(line, '\n')) != NULL) *pos = '\0'; //Take off the newline.
      if (strncmp(datname, "g1780", 9) == 0) {
        printf("data line for g1780 is |%s|\n", line); //line has a newline?
        fflush(NULL);
      }
      for (i = 0; i < refnLoci; i++) {
        ...
        for (j = 0; j < plantsTotal; j++) {
          if (line[j] == '-' || refchdata[i][j] == '-') nmissing++;
          else { //Each datum must be in capital case.
            if (line[j] != refchdata[i][j]) recombfrac++;
            if (line[j] == 'A' && refchdata[i][j] == 'B') recombfrac++;
            if (line[j] == 'B' && refchdata[i][j] == 'A') recombfrac++;
          }
        }
        recombfrac /= (2 * (plantsTotal - nmissing));
        if (recombfrac < minrf) {
          minrf = recombfrac;
          savedgroup = refdomains[i];
          strcpy(savedname, refnames[i]);
        }
        /*if (strncmp(datname, "g1780", 9) == 0) {
          printf("currdiff = %d mindiff = %d refnames[%d] = %s refdomains[%d] = %d refchdata[%d] = %s\n", currdiff, mindiff, i, refnames[i], i, refdomains[i], i, refchdata[i]);
        }*/
      }
      if (savedgroup < 0) printf("marker %s was not matched to any group\n", datname);
      else if (savedgroup < N) fprintf(OUD[savedgroup], "*%s\n%s\n", datname, line);
      k++;
    }
  }
}
