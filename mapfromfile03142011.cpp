...
int main(int argc, char *argv[]) {
  ...
  char *line, *token, *lctoken, *queryfile, vars[5][512], **names, *cfgline, *printstring;
  char *tempname, mmdatatype[20], *markernamesfile, *rfracfile, *filler, **chdata, *usedcodefile, *segdisfile, *dedupoutfile;
  int savedj, imax, kmax, queryflag, queryarray[20], *querycodes, mapqueries[20], mq, *foretestins, *afttestins;
  ...
  int g, m, n, km, nfac, mini, savedv[20], maxnamelength, iv, outaflag, *membership, highindex, lowcount, highcount;
  int *fracUsedCode, isfull, maxprdlinelength, maxdatlinelength, *preDosage, *postDosage, **nMissing, *plantsUsed;
  int **orthodata, *tempdata, *sectempdata, found, temp, found1, found2, found3, measflag, *highlowstatus, lowBs;
  int *lgStarts, *lgEnds, *lgCounts, *sortedlgCounts, *doneLinkageGroups, nf[12], **actmap, totmeas, highAs, highBs, lowAs;
  int ia, ib, ic, id, ie, iF, ig, ih, ii, ij, ip, stemlocusa, stemlocusb, permuteon, ik, im, *mafts, *mfores;
  int rfileflag, specialfileflag, nameflag, claimedloci, claimedplants, testlocusa, testlocusb, As, Bs, missingmeas;
  int flankfore, flankaft, anneallength, foreendm, aftendm, *insertions, savedfore, savedaft, instodo, pused, savedg;
  int gycount, gzcount, commit, rfxflag, combineparents, cllength, mincount, continuation, *lgnewCounts, *lgnewStarts;
  int *pool, *lgnewEnds, *sortedlgnewCounts, nfloop, totcount, currcount, printflag, dedupflag, exclusionflag;
  double segdisthreshold, *segdises, actlength, maxminrf, *minofrfs, **minlgdists, lowcutoff, highcutoff, *hightolows;
  double **recombfrac, mapLength, bandwidth, base, top, maxrf, queriedmaplength, duration, cudist;
  ...
  FILE *CFG, *OUT, *DAT, *PRD, *NGR, *SPE, *QRY, *ACT, *MNA, *RFF, *PUD, *SGD, *DDP, *MEA;
  if(strcmp(argv[1], "-") == 0 || strcmp(argv[1], "-h") == 0 || strcmp(argv[1], "-help") == 0) {
    ...
    printf(" should be done only if continuation is set to 2.  If nfloop is set, the second value of xxx should be set also.\n");
    return(0);
  }
  ...
  for (i = 0; i < nLoci; i++) names[i] = (char *) calloc(maxnamelength + 2, sizeof(char));
  ...
  if (measflag == 1) hightolows = (double *)calloc(nLoci + 10, sizeof(double));
  timeused1 = clock();
    ...
    if (strcmp(inputformat, "byplant") == 0 ) {
      ...
      if (strcmp(lctoken, "data") == 0) {
        ...
        if (strcmp(lctoken, "type") == 0) {
          ...
          lctoken[i] = '\0';
          if (strcmp(lctoken, "ri") == 0 || strcmp(lctoken, "f2") == 0 || strcmp(lctoken, "f3") == 0) {
            token = strtok(NULL, " \t\n");  
            i = 0;
            while (token[i] != '\0') {
              lctoken[i] = tolower(token[i]);
      ...
      if (combineparents == 1) {
        ...
        //orthodata has [plant][locus] for all plants and all loci, so it is possible to fill hightolows here.
        for (j = 0; j < nLoci; j++) {
          highAs = 0;
          highBs = 0;
          lowAs = 0;
  ...
  if (dedupflag > 0) {
    for (i = 0; i < nLoci; i++) {
      ...
      if (forePointers[i] == -1 && aftPointers[i] == -1) exemplars[i] = 1;
    }
    for (i = 0; i < nLoci; i++) {
    ...
    for (l = 0; l < nfloop; l++) {
      ...
      for (i = 0; i < nLoci; i++) {
        if (forePointers[i] == -1 && aftPointers[i] >= 0) {
          ...
          if (currcount < mincount) { //Dismember the too-short and presumed extraneous linkage group.
            g = i;
            while (g >= 0) {
              j = aftPointers[g];
              forePointers[g] = -1;
...
//          fprintf(OUT, "\n");
      ...
      for (i = 0; i < nLoci; i++) {
        ...
        if (found >= 1) {
          pool[k] = i;
          if (found == 1) {fprintf(OUT, "pool[%d] = %d, which is a fore end\n", k, pool[k]);} 
          if (found == 2) {fprintf(OUT, "pool[%d] = %d, which is an aft end\n", k, pool[k]);}
          k++;
        }
      }
      km = k;
              ...
              if ((forePointers[pool[k]] == -1 && aftPointers[pool[k]] >= 0) || (forePointers[pool[k]] >= 0 && aftPointers[pool[k]] == -1)) {
                if ((forePointers[pool[m]] == -1 && aftPointers[pool[m]] >= 0) || (forePointers[pool[m]] >= 0 && aftPointers[pool[m]] == -1)) {
                  ...
                  if (found == 0) { //We can anneal-join pool[k] and pool[m].
                    fprintf(OUT, "Loci %d and %d can be joined at rf = %.6f\n", pool[k], pool[m], recombfrac[pool[k]][pool[m]]);
            ...
            while (mergearray[kmax-1] >= 0) { 
              ...
              for (i = 0; i < nf[kmax]; i++) {
                ...
                if (localmaplength < minmaplength) {
                  ...
                  for (j = 0; j < kmax; j++) savedv[j] = mergearray[coeffs[kmax][i][j]];
                  if (found == 1) {
                    fprintf(OUT, "line 2320: %.6f", minmaplength);
                    for (j = 0; j < kmax; j++) fprintf(OUT, " %d", savedv[j]);
                    fprintf(OUT, "\n");
                  }
                }
              }
      ...
      if (continuation == 1) {
            ...
            for (j = 0; j < nLoci; j++) {
              if (recombfrac[i][j] < mindist) { 
                if (forePointers[j] >= 0 || aftPointers[j] >= 0) {
                  ...
                  savedj = j; //j belongs to a linkage group.
                }
              }
            }
            if (forePointers[savedj] == -1) { //savedj is a fore end
              temprfsj = recombfrac[savedj][aftPointers[savedj]] + recombfrac[savedj][aftPointers[aftPointers[savedj]]];
              temprfi = recombfrac[i][aftPointers[savedj]] + recombfrac[i][aftPointers[aftPointers[savedj]]];
              if (temprfsj < temprfi) { //Using this sum might clean up the end behavior of the linkage groups.
                aftPointers[i] = savedj;
                forePointers[savedj] = i;
                lgStarts[membership[savedj]] = i; //Update the start of the linkage group.
              }
              else {
                forePointers[aftPointers[savedj]] = i;
                aftPointers[i] = aftPointers[savedj];
                forePointers[i] = savedj;
                aftPointers[savedj] = i; //lgStarts[membership[savedj]] is unaffected.
              }
            }
            else {
              if (aftPointers[savedj] == -1) { //savedj is an aft end
                temprfsj = recombfrac[savedj][forePointers[savedj]] + recombfrac[savedj][forePointers[forePointers[savedj]]];
                temprfi = recombfrac[i][forePointers[savedj]] + recombfrac[i][forePointers[forePointers[savedj]]];
                if (temprfsj < temprfi) { //Using this sum might clean up the end behavior of the linkage groups.
                  aftPointers[savedj] = i;
                  forePointers[i] = savedj;
                  lgEnds[membership[savedj]] = i; //Update the end of the linkage group.
                }
                else {
                  aftPointers[forePointers[savedj]] = i;
                  forePointers[i] = forePointers[savedj];
                  aftPointers[i] = savedj;
                  forePointers[savedj] = i; //lgEnds[membership[savedj]] is unaffected.
                }
              }
              else { //savedj is within the linkage group
                trialdista = recombfrac[forePointers[savedj]][i] + recombfrac[savedj][aftPointers[savedj]];
                trialdistb = recombfrac[forePointers[savedj]][savedj] + recombfrac[i][aftPointers[savedj]];
        ...
        lgnewLengths = (double *) calloc(nLinkageGroups + 1, sizeof(double));
        lgnewCounts = (int *) calloc(nLinkageGroups + 1, sizeof(int));
        lgnewStarts = (int *) calloc(nLinkageGroups + 1, sizeof(int));
        lgnewEnds = (int *) calloc(nLinkageGroups + 1, sizeof(int));
        sortedlgnewCounts = (int *) calloc(nLinkageGroups + 1, sizeof(int));
        nLinkageGroups = 0; //Now update nLinkageGroups after all the new arrays have been allocated.
        k = 0;
        for (i = 0; i < nLoci; i++) {
          if (forePointers[i] == -1) {
            if (aftPointers[i] >= 0) {
        ...
        for (i = 0; i < nLinkageGroups; i++) {
          for (j = 0; j < nLinkageGroups; j++) {
            if (lgnewCounts[j] == sortedlgnewCounts[i] && doneLinkageGroups[j] == 0) {
  ...
  free(doneLinkageGroups); free(membership);
  for (i = 0; i < nLinkageGroups; i++) free(minlgdists[i]);
